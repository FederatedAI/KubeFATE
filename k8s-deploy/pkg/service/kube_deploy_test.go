...
func TestGetClusterDeployStatus(t *testing.T) {
	type args struct {
		name      string
		namespace string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for {
				got, err := GetClusterDeployStatus(tt.args.name, tt.args.namespace)
				if (err != nil) != tt.wantErr {
					t.Errorf("GetClusterDeployStatus() error = %v, wantErr %v", err, tt.wantErr)
					return
				}
				if !reflect.DeepEqual(got, tt.want) {
					t.Errorf("GetClusterDeployStatus() = %v, want %v", got, tt.want)
				}
