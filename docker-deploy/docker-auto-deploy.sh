...
cd ${WORKINGDIR}
rm -rf exchange/
mkdir -p exchange/conf
cp ${WORKINGDIR}/../.env exchange/
cp docker-compose-exchange.yml exchange/
cp -r docker-example-dir-tree/proxy/conf exchange/

sed -i.bak "s/port=.*/port=${proxy_port}/g" ./exchange/conf/proxy.properties
sed -i.bak "s#route.table=.*#route.table=${deploy_dir}/proxy/conf/route_table.json#g" ./exchange/conf/proxy.properties
sed -i.bak "s/coordinator=.*/coordinator=${party_id}/g" ./exchange/conf/proxy.properties
sed -i.bak "s/ip=.*/ip=0.0.0.0/g" ./exchange/conf/proxy.properties

cat > ./exchange/conf/route_table.json <<EOF
{
    "route_table": {
...
tar -czf ./exchange.tar ./exchange
scp -r ./exchange.tar $user@$exchangeip:~/
ssh -tt $user@$exchangeip<< eeooff
mv ~/exchange.tar $dir
cd $dir
tar -xzf exchange.tar
cd exchange
...
echo "party exchange deploy is ok!"